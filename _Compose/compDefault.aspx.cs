...
{
    public partial class _compDefault : System.Web.UI.Page
    {
        private MyApplicationPermission __gmap;

        protected void Page_PreInit(object sender, EventArgs e)
        {
            __gmap = new MyApplicationPermission(MyApplicationPermission.ApplicationName.Compose, this.Session["XIPS_ENV__USER_CREDENTIAL_CLASS"] as UserCredential);
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                this.ddlGridPageCount.SelectedIndex = this.Session["COMPOSEMASTER_LIST__GRID_PAGECOUNT__SINX"] == null ? 1 : int.Parse(this.Session["COMPOSEMASTER_LIST__GRID_PAGECOUNT__SINX"].ToString());
            ...
            try
            {
                switch (e.CommandName)
                {
                    case "EditData":
                        ...
                        {
                            Response.Redirect(string.Format("../_Compose/compDetail.aspx?Mode=Update&ComposeID={0}", e.CommandArgument));
                        }
                        else
                        {
                    ...
                    case "DeleteData":
                        if (__gmap.RightToDelete == MyApplicationPermission.PermissionLevel.Yes)
                        {
                            ...
                            {
                                if (__xips.DeleteComposeSet(e.CommandArgument.ToString()))
                                {
                                     this.btnRefresh_Click(sender, e);
                                }
                                else
                                {
                                    throw new Exception( "刪除資料失敗. " + __xips.ErrorMessage);
                                }
                            }
                        }
                        else
                        ...
                        break;
                    case "Detail":
                        Response.Redirect(string.Format("../_Compose/compDetail.aspx?Mode=Detail&ComposeID={0}", e.CommandArgument));
                        break;
                    case "sort":
                        break;
                    case "Page":
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("../_Compose/compDetail.aspx?Mode=Insert");
            }
            else
            {
                this.pnlErrorMsg.Visible = true;
        ...
        {
            this.Session["COMPOSEMASTER_LIST__GRID_PAGECOUNT__SINX"] = this.ddlGridPageCount.SelectedIndex;
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (e.Row.Cells[3].Text.Length > 80)
                    e.Row.Cells[3].Text = e.Row.Cells[3].Text.Substring(0, 80) + "...";
            }
        }
    }
}