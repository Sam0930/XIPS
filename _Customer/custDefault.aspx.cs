...
namespace XIPS._Customer
{
    public partial class _custDefault : System.Web.UI.Page
    {
        ...
        {
            __gmap = new MyApplicationPermission(MyApplicationPermission.ApplicationName.Customer, this.Session["XIPS_ENV__USER_CREDENTIAL_CLASS"] as UserCredential);
        }
            ...
            try
            {
                switch (e.CommandName)
                {
                    case "EditData":
                        ...
                        {
                            Response.Redirect(string.Format("../_Customer/custDetail.aspx?Mode=Update&CustomerID={0}", e.CommandArgument,ToString()));
                        }
                        else
                        {
                    ...
                    case "DeleteData":  
                        if (__gmap.RightToDelete == MyApplicationPermission.PermissionLevel.Yes)
                        {
                            ...
                            {
                                // Validate to delete Customer.
                                if (!__xips.ValidateToDeleteCustomerInformation(e.CommandArgument.ToString()))
                                    throw new Exception(Resources.DefaultResource.MSG__ERROR_DELETE_DATA_INUSE);

                                if (__xips.DeleteData(XipsDAL.DataType.Customer, e.CommandArgument.ToString()))
                                {
                        ...
                        break;
                    case "Detail":
                        Response.Redirect(string.Format("../_Customer/custDetail.aspx?Mode=Detail&CustomerID={0}", e.CommandArgument));
                        break;
                    case "sort":
                        //Do nothing here
                        break;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("../_Customer/custDetail.aspx?Mode=Insert");
            }
            else
            {
                this.pnlErrorMsg.Visible = true;
